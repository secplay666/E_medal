...
class BleDebugFragment : Fragment() {
            ...
            object : BluetoothGattCallback() {
                ...
                override fun onCharacteristicWrite(
                    ...
                    if (status == BluetoothGatt.GATT_SUCCESS) {
                        ...
                        // 写入成功，通知全局 manager 继续队列
                        handler.post {
                            BleConnectionManager.onCharacteristicWrite(status)
                        }
                    } else {
                        Log.w(TAG, "写入特征值失败，状态: $status")
                        appendToReceiveBox("发送失败")
                        // 出现写入失败，通知 manager 以便其处理重试或错误记录
                        handler.post {
                            BleConnectionManager.onCharacteristicWrite(status)
    ...
    private fun writeCharacteristicData(data: String) {
        ...
        try {
            ...
            // 委托给全局连接管理器进行分片/CRC/重试发送
            BleConnectionManager.sendDataWithFragments(bytes)

            Log.d(TAG, "准备发送数据(已交给 manager): $data")
            appendToReceiveBox("准备发送(交由 manager): $data")
        } catch (e: SecurityException) {
    ...
    // 分片/CRC/发送逻辑由 BleConnectionManager 提供
    ...
    override fun onDestroyView() {
        super.onDestroyView()
        // 不在 fragment 销毁时自动断开连接，连接由 BleConnectionManager 管理
    }
